...
class Cat(pg.sprite.Sprite):
    ...
    def move(self, dt):
        ...
        if keys[pg.K_w] and self.position.y > 100:
...
class Game:
    def __init__(self):
        ...
        self.win_text = self.win_font.render("WIN", True, "red")
        self.win_rect = self.win_text.get_rect()
        self.win_rect.center = (1000 // 2, 700 // 2)
    ...
    def check_collision(self):
        collisions = pg.sprite.groupcollide(
            self.cat_group, self.food_group, False, True
        )
        if collisions:
            food_list = next(iter(collisions.values()))
            food = food_list[0]
            color_name, point_value = food.color
            self.score += point_value
            self.food_group.add(Food(200, 200, ("red", -1)))
